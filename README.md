...
After running `eval_real.py` to generate predictions, you can visualize them on a 3D brain surface using MATLAB.
...
- `visualize_transformer_predictions.m` - Main visualization script for multiple samples
- `visualize_single_prediction.m` - Quick single sample viewer
- `compare_predictions.m` - Compare predictions from different models
- `inspect_anatomy_data.m` - Explore anatomy data structure
- `test_visualization.m` - Test visualization setup

### Requirements

The visualization requires:
- MATLAB (tested with R2020a+)
- Anatomy files in `anatomy/` folder:
...
For detailed visualization instructions, see:
- `misc_scripts/VISUALIZATION_GUIDE.md` - Comprehensive guide
- `misc_scripts/README_VISUALIZATION.md` - Script documentation
...
- Run `test_visualization.m` in MATLAB to check setup
- Ensure anatomy files exist in `anatomy/` folder
- Make sure prediction files were generated by `eval_real.py`
- Check that you're in the `misc_scripts` directory when running scripts
